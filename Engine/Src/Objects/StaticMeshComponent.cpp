...
namespace Ailu
{
	...
	void StaticMeshComponent::Tick(const float& delta_time)
	{
		...
		if (_p_mats.size() != _p_mesh->SubmeshCount())
		{
			...
			{
				_p_mats.emplace_back(g_pResourceMgr->GetRef<Material>(ToWStr(it->_name.c_str())));
			}
		}
	}
	...
	void StaticMeshComponent::Serialize(std::basic_ostream<char, std::char_traits<char>>& os, String indent)
	{
		...
		os << prop_indent << "MeshPath: " << (_p_mesh ? ToChar(ResourceMgr::GetObjectSysPath(_p_mesh.get())) : String("")) << endl;
	...
	void* StaticMeshComponent::DeserializeImpl(Queue<std::tuple<String, String>>& formated_str)
	{
		...
		else
		{
			...
			if (!mesh_path.empty())
			{
				mesh = g_pResourceMgr->GetRef<Mesh>(ToWStr(mesh_path.c_str()));
				if (mesh == nullptr)
				{
	...
	void* SkinedMeshComponent::DeserializeImpl(Queue<std::tuple<String, String>>& formated_str)
	{
		...
		else
		{
			...
			formated_str.pop();
			auto mat_path = TP_ONE(formated_str.front());
			Ref<Mesh> mesh = g_pResourceMgr->GetRef<Mesh>(ToWStr(mesh_path.c_str()));
			if (mesh == nullptr)
			{
			...
			auto mat = g_pResourceMgr->LoadAsset<Material>(Guid(mat_path));
