...
namespace Ailu
{
	...
	ShadowCastPass::ShadowCastPass() : RenderPass("ShadowCastPass"), _rect(Rect{ 0,0,kShadowMapSize,kShadowMapSize })
	{
		...
		_p_shadowcast_material = MakeRef<Material>(g_pResourceMgr->Load<Shader>(L"Shaders/depth_only.hlsl__"), "ShadowCast");
	}
	...
	DeferredGeometryPass::DeferredGeometryPass(u16 width, u16 height) : RenderPass("DeferedGeometryPass")
	{
		...
		_p_lighting_material = MakeRef<Material>(g_pResourceMgr->Load<Shader>(L"Shaders/deferred_lighting.hlsl", &setting), "DeferedGbufferLighting");
		_p_quad_mesh = Mesh::s_p_quad;
		for (int i = 0; i < _rects.size(); i++)
			_rects[i] = Rect(0, 0, width, height);
		_p_ibllut = g_pResourceMgr->Get<Texture2D>(EnginePath::kEngineTexturePathW + L"ibl_brdf_lut.png");
	}
	...
	SkyboxPass::SkyboxPass() : RenderPass("SkyboxPass")
	{
		...
		_p_skybox_material = MakeRef<Material>(g_pResourceMgr->Load<Shader>(L"Shaders/skybox.hlsl"), "Skybox");
