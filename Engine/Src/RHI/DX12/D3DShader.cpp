...
namespace Ailu
{
	...
	static EShaderDateType GetShaderDataType(const char* semantic)
	{
		...
		else if (!std::strcmp(semantic, RenderConstants::kSemanticVertexIndex)) return EShaderDateType::kNone;
		else
		{
	...
	void D3DComputeShader::BindOnly(CommandBuffer* cmd, u16 thread_group_x, u16 thread_group_y, u16 thread_group_z, std::function<void()> res_binder)
	{
		...
		for (auto& info : _bind_res_infos)
		{
			...
			if (bind_info._p_res != nullptr)
			{
				...
				{
					reinterpret_cast<IConstantBuffer*>(bind_info._p_res)->Bind(cmd, bind_info._bind_slot);
				}
			}
		}
		d3dcmd->Dispatch(thread_group_x, thread_group_y, thread_group_z);
	...
	void D3DComputeShader::LoadReflectionInfo(ID3D12ShaderReflection* p_reflect)
	{
		...
		for (u32 i = 0u; i < desc.ConstantBuffers; i++)
		{
			...
			for (u32 j = 0u; j < desc.Variables; j++)
			{
				...
				auto info = ShaderBindResourceInfo{ value_type,variable_info,0u,vdesc.Name };
				info._p_root_cbuf = &_temp_bind_res_infos.find(desc.Name)->second;
	...
	bool D3DComputeShader::RHICompileImpl()
	{
		...
		try
		{
			...
			LoadShaderReflection(_p_reflection.Get());
#else
			succeed &= CreateFromFileFXC(_src_file_path, "cs_main", RenderConstants::kCSModel_5_0, v, _tmp_blob, _p_reflection,_all_dep_file_pathes);
#endif // SHADER_DXC
		}
		catch (const std::exception&)
		{
