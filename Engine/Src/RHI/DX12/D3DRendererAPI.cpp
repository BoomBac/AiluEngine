...
namespace Ailu
{
	...
	void D3DRendererAPI::DrawIndexedInstanced(const std::shared_ptr<IndexBuffer>& index_buffer, uint32_t instance_count)
	{
		D3DContext::GetInstance()->GetCmdList()->DrawIndexedInstanced(index_buffer->GetCount(), instance_count, 0, 0, 0);
	}
	void D3DRendererAPI::DrawInstanced(const std::shared_ptr<VertexBuffer>& vertex_buf, uint32_t instance_count)
	{
