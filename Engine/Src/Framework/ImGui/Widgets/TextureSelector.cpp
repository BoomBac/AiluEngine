...
namespace Ailu
{
	...
	void TextureSelector::ShowImpl()
	{
		...
		int tex_count = 0, tex_num = g_pTexturePool->Size();
		for (auto& [tex_name, tex] : *g_pTexturePool)
		{
			if (tex->Dimension() == ETextureDimension::kTex2D)
			{
				...
				if (ImGui::ImageButton(TEXTURE_HANDLE_TO_IMGUI_TEXID(tex->GetNativeTextureHandle()), ImVec2(preview_tex_size, preview_tex_size), uv0, uv1, 0))
				{
	...
	const WString& TextureSelector::GetSelectedTexture(i32 handle) const
	{
