#ifndef __COLOR_SPACE_UTILS
#define __COLOR_SPACE_UTILS
//
...
float3 ApplySRGBCurve_Fast( float3 x )
{
    ...
    #if defined(SHADER_DXC)
        return select(12.92 * x, 1.13005 * sqrt(x - 0.00228) - 0.13448 * x + 0.005719, x >= 0.0031308);
    #else
        return x < 0.0031308 ? 12.92 * x : 1.13005 * sqrt(x - 0.00228) - 0.13448 * x + 0.005719;
    #endif
}
...
float3 RemoveSRGBCurve_Fast( float3 x )
{
    ...
    #if defined(SHADER_DXC)
        return select(x / 12.92, -7.43605 * x - 31.24297 * sqrt(-0.53792 * x + 1.279924) + 35.34864, x >= 0.04045);
    #else
        return x < 0.04045 ? x / 12.92 : -7.43605 * x - 31.24297 * sqrt(-0.53792 * x + 1.279924) + 35.34864;
    #endif
}
...
float3 ApplyREC709Curve( float3 x )
{
    ...
    #if defined(SHADER_DXC)
        return select(4.5 * x, 1.0993 * pow(x, 0.45) - 0.0993, x >= 0.0181);
    #else
        return x < 0.0181 ? 4.5 * x : 1.0993 * pow(x, 0.45) - 0.0993;
    #endif
}
...
float3 RemoveREC709Curve( float3 x )
{
    ...
    #if defined(SHADER_DXC)
        return select(x / 4.5, pow((x + 0.0993) / 1.0993, 1.0 / 0.45), x >= 0.08145);
    #else
        return x < 0.08145 ? x / 4.5 : pow((x + 0.0993) / 1.0993, 1.0 / 0.45);
    #endif
}
