...
void RTXDI_StoreDIReservoir(const RTXDI_DIReservoir reservoir,uint handle,int index)
{
    if (valid_bindless_handle(handle))
        BINDLESS_RWBUFFER_STORE_INDEX(RTXDI_PackedDIReservoir, handle, index, RTXDI_PackDIReservoir(reservoir));
}
...
RTXDI_DIReservoir RTXDI_LoadDIReservoir(uint handle,int index)
{
    if (valid_bindless_handle(handle))
        return RTXDI_UnpackDIReservoir(BINDLESS_RWBUFFER_LOAD_INDEX(RTXDI_PackedDIReservoir, handle, index));
    else
